...
from utils.utils import get_sem, mean_confidence_interval
from utils.utils import gcn_norm_fill_val
...
class ODEFunc(nn.Module):
  ...
  def __init__(self, in_features, out_features, opt, adj):
    ...
    if isinstance(self.adj, torch.Tensor):
        ...
        self.alpha_train = nn.Parameter(self.alpha * torch.ones(adj.size(1)))

    self.w = nn.Parameter(torch.eye(opt['hidden_dim']))
    self.d = nn.Parameter(torch.zeros(opt['hidden_dim']) + 1)

  def reset_parameters(self):
    nn.init.eye_(self.w)  # Сбрасываем веса в единичную матрицу
    nn.init.ones_(self.d)  # Сбрасываем смещение в 1
  
  def forward(self, t, x):
    self.nfe += 1
    alph = torch.sigmoid(self.alpha_train).unsqueeze(dim=1)
    if isinstance(self.adj, torch.Tensor):  
      ax = torch.spmm(self.adj, x)  # Обычный sparse_coo_tensor
    else:  
      ax = self.adj.matmul(x)
...
class ODEblock(nn.Module):
  ...
  def reset_parameters(self):
    if hasattr(self.odefunc, 'reset_parameters'):
      self.odefunc.reset_parameters()

  def forward(self, x):

    t = self.t.type_as(x)
    z = odeint(self.odefunc, x, t, method="rk4")[1]
...
class CGNN(nn.Module):
  def __init__(self, opt, adj, time, device):
    ...
    self.device = device
    if opt['beltrami']:
      self.mx = nn.Linear(opt['num_feature'], opt['feat_hidden_dim'])
  ...
  def reset_parameters(self):
    if hasattr(self.odeblock, 'reset_parameters'):
      self.odeblock.reset_parameters()
      if self.opt['beltrami']:
        self.mx.reset_parameters()
      else:
  ...
  def forward(self, x, pos_enc=None):
    ...
    if self.opt['beltrami']:
      ...
      if self.opt['dataset'] == 'ogbn-arxiv':
        ...
        p = F.dropout(pos_enc, self.opt['input_dropout'], training=self.training)
        p = self.mp(p)
