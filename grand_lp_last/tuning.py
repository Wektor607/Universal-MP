    ...
    for run in range(args.runs):
        ...
        hyperparameter_search = {
                                ...
                                #  "batch_size": [8192, 16384, 32768, 65536], 
                                 "gnnlr": [0.1, 0.01, 0.001, 0.0001, 0.00001], 
                                 "prelr": [0.1, 0.01, 0.001, 0.0001, 0.00001],
                                #  "ttime":[2, 2.5, 3, 3.5], "tol_scale": [100, 1000, 5000, 10000],
                                #  "tol_scale_adjoint": [100, 1000, 5000, 10000],
                                 }
        # for combo_i, (hiddim, predp, preedp, batch_size, gnnlr, prelr, ttime, tol_scale, tol_scale_adjoint) in \
        for combo_i, (hiddim, gnnlr, prelr) in \
            enumerate(tqdm(itertools.product(*hyperparameter_search.values()),
                   desc="Grid-search")):
            # combo_label = (
            #     f"hiddim={hiddim}_predp={predp}_preedp={preedp}_"
            #     f"bs={batch_size}_gnnlr={gnnlr}_prelr={prelr}"
            # )
            # opt['time'] = ttime
            # opt['tol_scale'] = tol_scale
            # opt['tol_scale_adjoint'] = tol_scale_adjoint
            combo_label = (
                # f"time={ttime}_tol_scale={tol_scale}_tol_scale_adjoint={tol_scale_adjoint}_"
                # f"bs={batch_size}_gnnlr={gnnlr}_prelr={prelr}"
                f"gnnlr={gnnlr}_prelr={prelr}"
            )
            
            wandb.init(project="GRAND4LP_new", name=combo_label, 
                       group="grid_search", reinit=True)

            opt['feat_hidden_dim'] = hiddim // 2
            opt['pos_enc_hidden_dim'] = hiddim // 2

            if opt['beltrami']:
                opt['pos_enc_type'] = f'DW{hiddim}'
            ...
            predictor = predfn(opt['hidden_dim'], opt['hidden_dim'], 1, opt['num_layers'],
                                opt['predp'], opt['preedp'], args.lnnn).to(device)
                
            # model = GNN_KNN(opt, data, splits, batch_size, device).to(device) 
            ...
            for name, param in predictor.named_parameters():
                if param.requires_grad:
                    print(f"{name}, {param.shape}")
            optimizer = torch.optim.Adam([{'params': model.parameters(), "lr": gnnlr}, 
                                        {'params': predictor.parameters(), 'lr': prelr}])
            ...
            for epoch in tqdm(range(1, opt['epoch'] + 1)):
                ...
                # wandb.log({f"train_loss/{combo_label}": loss}, step=epoch)
                wandb.log({
                    "train_loss": loss,  # Одна общая метрика для всех конфигураций
                    "config": combo_label  # Подпись параметров
                }, step=epoch)
                ...
                if epoch % args.eval_steps == 0:
                    ...
                    for key, result in results.items():
                        ...
                        wandb.log({
                            key: result[-1],  # Логируем метрику под одним ключом
                            "config": combo_label  # Подпись конфигурации параметров
                        }, step=epoch)
