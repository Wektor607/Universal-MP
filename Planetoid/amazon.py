...
def test(model, predictor, data, split_edge, evaluator, batch_size):
    ...
    train_result = torch.cat((torch.ones(pos_train_pred.size()), torch.zeros(neg_valid_pred.size())), dim=0)
    ...
    results['AUC'] = (roc_auc_score(train_result.cpu().numpy(), train_pred.cpu().numpy()), roc_auc_score(valid_result.cpu().numpy(), 
                    valid_pred.cpu().numpy()), roc_auc_score(test_result.cpu().numpy(), test_pred.cpu().numpy()))
